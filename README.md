...
资源库内置硬链接：多个版本、语言或可见目录中的音频可能共享同一份底层内容。**需要修改音频时，
请先复制到资源库外，或通过导出生成独立副本，再编辑副本。不要直接在库内目录编辑文件，否则
可能同时修改共享原文件和其他目录中的音频。** 硬链接不可用时会明确报错并保留原文件，不生成独立媒体副本。
新音频按版本、语言、实体、皮肤和音频类型分组，类别目录内直接保存原始 ID 文件；“按类型分组输出”开关继续保留，默认按实体分组。
启动时会一次性迁移旧版无语言层的输出：按实际 WEM 字节哈希归档内容，建立新目录硬链接和索引，同时搬迁已有映射、元数据与 WAV。完成后移除旧位置，不保留两套输出；无需重新解包。迁移失败会保留尚未处理完的旧文件并阻止新任务，处理原因后重新启动可继续。
...
关闭通知后仍能从执行中心的“查看本次结果”恢复。转码文件任务失败时自动重试，默认最多尝试 3 次（含首次），保持原后端和参数，不重复工具预检。耗尽次数后保留失败诊断，供用户处理后重新创建任务。

皮肤和炫彩按实际资源共享关系处理：纯继承事件不重复展开，有变化的事件保留完整音频列表，
共享 WEM 只在来源皮肤下输出一份。相同内容只转码一次，库内 WAV 按内容与转换方案校验后复用；
...
显式离线转换；工具不覆盖已有文件。启动时的一次性旧目录迁移也会转换其中的结构化数据，正常读取不回退旧格式。完整类型与失败边界见 [API 数据格式约定](docs/api/README.md#4-数据格式约定)。
