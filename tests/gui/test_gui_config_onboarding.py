...
@pytest.mark.parametrize("mark_method", ["mark_onboarding_completed", "mark_onboarding_skipped"])
def test_current_onboarding_state_hides_guide(mark_method: str) -> None:
    """完成或跳过当前版本后都不应再次展示。"""
    cfg = GuiConfig()

    getattr(cfg, mark_method)(GUIDE_VERSION)

    assert cfg.should_show_onboarding(GUIDE_VERSION) is False


@pytest.mark.parametrize("mark_method", ["mark_onboarding_completed", "mark_onboarding_skipped"])
def test_reset_current_onboarding_state_shows_guide(mark_method: str) -> None:
    """重置完成或跳过状态后应允许下次启动展示。"""
    cfg = GuiConfig()
    getattr(cfg, mark_method)(GUIDE_VERSION)
...
@pytest.mark.parametrize("mark_method", ["mark_onboarding_completed", "mark_onboarding_skipped"])
def test_onboarding_state_persists_to_project_ini(tmp_path: Path, mark_method: str) -> None:
    """完成或跳过状态应写入项目 INI 并能跨实例读取。"""
