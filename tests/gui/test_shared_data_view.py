...
def test_shared_data_display_uses_real_progress_counts() -> None:
    ...
    unknown = describe_shared_data_state(SharedDataState(SharedDataPhase.CHECKING, 3, "remote_snapshot"))
...
def test_shared_data_display_ready_and_partial_share_authoritative_summary() -> None:
    ...
    ready = describe_shared_data_state(SharedDataState(SharedDataPhase.READY, 4, "local_path", summary=summary))
    partial = describe_shared_data_state(SharedDataState(SharedDataPhase.PARTIAL, 4, "local_path", summary=summary))
...
def test_shared_data_display_escalates_persisting_schema_problem_to_regenerate() -> None:
    ...
    initial = describe_shared_data_state(SharedDataState(SharedDataPhase.PARTIAL, 5, "local_path", problem=problem))
