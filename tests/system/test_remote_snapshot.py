...
pytestmark = [pytest.mark.system, pytest.mark.remote_live]
...
CLI_TIMEOUT = int(os.environ.get("LOL_REMOTE_SMOKE_TIMEOUT", "900"))
...
def _resolve_version_root(output_path: Path) -> Path:
    """返回远端 smoke 生成的版本目录。
...
def test_remote_cli_extracts_champion_vo(tmp_path: Path) -> None:
    ...
    version_root = _resolve_version_root(output_path)
    ...
    champion_dirs = [
        path for path in champion_root.iterdir() if path.is_dir() and path.name.startswith(f"{CHAMPION_ID}·")
    ]
