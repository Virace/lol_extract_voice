...
_TEXTS: dict[str, dict[str, str]] = {
    "zh_CN": {
        ...
        "help.config_file": "启用绝对独占的配置文件模式；仅允许同时提供动作列表和配置文件路径。仅写 -c 时读取默认 INI，写 -c PATH 时读取指定 INI。",
        ...
        "help.actions": "要执行的动作列表，支持顺序提供多个动作，如 `update extract`。",
        "help.mapping.integrate_data_global": "mapping 阶段是否生成整合数据文件；未显式指定时默认开启。",
        "help.wav": "在解包后并行派生 WAV 输出。",
        "help.wav_workers": "设置 WAV 转码并发进程数。",
        ...
        "stage.extract": "音频解包",
        "stage.mapping": "事件映射",
        "error.actions.required": "错误：必须提供至少一个动作：update / extract / mapping。",
